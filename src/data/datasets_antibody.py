...
def _extract_row_sample_id(row: pd.Series) -> int | None:
    if "sample_id" in row.index and pd.notna(row["sample_id"]):
        try:
            return int(row["sample_id"])
        except Exception:
            return None
    if "pdb_name" in row.index:
        name = str(row["pdb_name"])
        match = re.search(r"(?:_sample_)?(\d+)$", name)
        if match:
            try:
...
class AntibodyTestDataset(Dataset):
    def __init__(self, dataset_cfg, task):
        ...
        override_set = {int(x) for x in sample_ids_override} if sample_ids_override is not None else None

        all_sample_ids = []
        for row_id in range(len(self._benchmark_df)):
            target_row = self._benchmark_df.iloc[row_id]
            row_sample_id = _extract_row_sample_id(target_row)
            if row_sample_id is not None and self.samples_per_target == 1:
                if override_set is not None and row_sample_id not in override_set:
                    continue
                all_sample_ids.append((target_row, row_sample_id))
            else:
                for sample_id in range(self.samples_per_target):
                    if override_set is not None and sample_id not in override_set:
                        continue
    ...
    def __getitem__(self, idx):
        ...
        output_feats = {
            ...
            "res_idx": feats.get("res_idx"),
            "original_res_idx": feats.get("original_res_idx"),
        }
