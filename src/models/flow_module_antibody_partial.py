    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        for attempt in range(attempt_num):
            ...
            for i in range(num_batch):
                ...
                pdb_path = os.path.join(save_dir, f"sample{int(sample_id)}.pdb")
                ...
                last_pdb_path = pdb_path

            if last_pdb_path is None:
                last_pdb_path = os.path.join(save_dir, f"sample{batch_idx}.pdb")
            total_breaks, _ = pdb_clash.detect_breaks_in_structure(last_pdb_path, max_peptide_bond_length=1.6)
            clash = pdb_clash.detect_backbone_clash(last_pdb_path)
