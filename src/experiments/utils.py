...
class ScaffoldingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        motif_locations = save_motif_segments.motif_locations_from_contig(sample_contig[0])#[(18, 37), (53, 72)]
...
def get_sampled_mask(contigs, length, rng=None, num_tries=1000000):
    ...
    return sampled_mask, sampled_mask_length, inpaint_chains #sample的总长度，包括motif在内


def dataset_creation(dataset_class, cfg, task, validation_dataset_class=None):
    if validation_dataset_class is None:        # add-mono
        validation_dataset_class = dataset_class
    train_dataset = dataset_class(
        dataset_cfg=cfg,
        task=task,
        is_training=True,
    )
    eval_datasets = []    # * multi-val
    eval_datasets.append(validation_dataset_class(     # add-mono
        dataset_cfg=cfg,
        task=task,
        is_training=False,
    ))
    if cfg.val_csv_path1 != 'unused':
        eval_datasets.append(validation_dataset_class(     # add-mono
            dataset_cfg=cfg,
            task=task,
            is_training=False,
            is_additional_val=True
        ))
    return train_dataset, eval_datasets

def test_dataset_creation(dataset_class, cfg, task):
